...
const set_char_at = (str, i, c) =>
  str.slice(0, i) + c + str.slice(i + 1);
...
const guess_game = (game, alphabet) => {
  ...
  if (last_word == next_word) {
    ...
    game.last_word = next_word;
    game.last_message = '맞췄습니다!';
    if (next_word == word) {
      game.is_over = true;
      game.last_message = '승리!';
    }
  }
};
...
const on_message = msg => {
  ...
  if (games[room] === undefined) {
    if (content == 'hm') {
      game = create_game();
      games[room] = game;
      msg.reply(render_game(game));
    }
  }
  else if (/^[a-z]$/.test(content)) {
    game = games[room];
    guess_game(game, content);
    msg.reply(render_game(game));
    if (game.is_over) {
      delete games[room];
    }
...
BotManager.getCurrentBot().addListener(Event.MESSAGE, on_message);
